...
Figure6 <- function( sigmaMax=16, x0=15 ){
	...
	# Mark the four points on the graph by number (corresponding to the notional uncertainty-gradient graph)
	#  in the previous section of the paper
	text(4,13,labels=c("4"),cex=1.5) #expression(paste("4: Defined By Initial ",Delta,"x"))))
	...
	par(mar=c(0, 0, 0, 0))
	plot.new()
	legend("top", c("1 review & 1 test, both 0.01% effective","1 review & 1 test, both 99.99% effective"), 
		pch=c(15,16), horiz=TRUE, bty="n", cex=0.85)	
	legend("center", c("10 reviews & 10 tests, both 0.01% effective","10 reviews & 10 tests, both 99.99% effective"), 
